...
import {
  ...
  IDE_DEFINITIONS,
  type IdeInfo,
} from '@pulsai/nika-cli-core/src/ide/detect-ide.js';

const CLI_IDE_COMPANION_IDENTIFIER = 'Google.nika-cli-vscode-ide-companion';
  ...
  try {
    ...
    if (
      ...
      const selection = await vscode.window.showInformationMessage(
        `A new version (${latestVersion}) of the Nika CLI Companion extension is available.`,
...
export async function activate(context: vscode.ExtensionContext) {
  logger = vscode.window.createOutputChannel('Nika CLI IDE Companion');
  ...
  if (
    ...
    void vscode.window.showInformationMessage(
      'Nika CLI Companion extension successfully installed.',
    );
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand('nika-cli.runNikaCLI', async () => {
      const workspaceFolders = vscode.workspace.workspaceFolders;
      if (!workspaceFolders || workspaceFolders.length === 0) {
        vscode.window.showInformationMessage(
          'No folder open. Please open a folder to run Nika CLI.',
        );
        return;
      }
      ...
      if (workspaceFolders.length === 1) {
        ...
        selectedFolder = await vscode.window.showWorkspaceFolderPick({
          placeHolder: 'Select a folder to run Nika CLI in',
        });
      }
      ...
      if (selectedFolder) {
        ...
        const terminal = vscode.window.createTerminal({
          name: `Nika CLI (${selectedFolder.name})`,
    ...
    vscode.commands.registerCommand('nika-cli.showNotices', async () => {
