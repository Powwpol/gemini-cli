...
async function getDocsRoot(): Promise<string> {
  ...
  throw new Error('Could not find Nika CLI documentation directory.');
}
  ...
  async execute(_signal: AbortSignal): Promise<ToolResult> {
    try {
      ...
      if (!this.params.path) {
        ...
        const resultContent = `Available Nika CLI documentation files:\n\n${fileList}`;
...
/**
 * A tool that provides access to Nika CLI's internal documentation.
...
> {
  ...
  constructor(messageBus: MessageBus) {
    super(
      ...
      'GetInternalDocs',
      'Returns the content of Nika CLI internal documentation files. If no path is provided, returns a list of all available documentation paths.',
      Kind.Think,
      {
        type: 'object',
        properties: {
