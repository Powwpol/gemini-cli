...
describe('memory commands', () => {
  ...
  describe('listMemoryFiles', () => {
    it('should list the memory files in use', () => {
      const filePaths = ['/path/to/NIKA.md', '/other/path/NIKA.md'];
      ...
      if (result.type === 'message') {
        expect(result.messageType).toBe('info');
        expect(result.content).toContain('There are 2 NIKA.md file(s) in use:');
    ...
    it('should show a message if no memory files are in use', () => {
      ...
      if (result.type === 'message') {
        expect(result.messageType).toBe('info');
        expect(result.content).toBe('No NIKA.md files in use.');
      }
    });
