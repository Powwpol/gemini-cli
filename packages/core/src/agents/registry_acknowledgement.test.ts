...
describe('AgentRegistry Acknowledgement', () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'nika-cli-test-'));

    // Override NIKA_CLI_HOME to point to the temp directory
    originalGeminiCliHome = process.env['NIKA_CLI_HOME'];
    process.env['NIKA_CLI_HOME'] = tempDir;
  ...
  afterEach(async () => {
    ...
    if (originalGeminiCliHome) {
      process.env['NIKA_CLI_HOME'] = originalGeminiCliHome;
    } else {
