...
const CliHelpReportSchema = z.object({
  answer: z
    .string()
    .describe('The detailed answer to the user question about Nika CLI.'),
  sources: z
    .array(z.string())
...
/**
 * An agent specialized in answering questions about Nika CLI itself,
...
export const CliHelpAgent = (
  ...
  description:
    'Specialized in answering questions about how users use you, (Nika CLI): features, documentation, and current runtime configuration.',
  inputConfig: {
    inputSchema: {
      type: 'object',
      properties: {
        question: {
          type: 'string',
          description: 'The specific question about Nika CLI.',
        },
      },
      required: ['question'],
    },
  ...
  promptConfig: {
    query:
      'Your task is to answer the following question about Nika CLI:\n' +
      '<question>\n' +
      '${question}\n' +
      '</question>',
    systemPrompt:
      "You are **CLI Help Agent**, an expert on Nika CLI. Your purpose is to provide accurate information about Nika CLI's features, configuration, and current state.\n\n" +
      '### Runtime Context\n' +
