    ...
    const mockReaddir = vi.fn(
      async (
        ...
        if (
          ...
          nodePath
            .normalize(path)
            .includes(nodePath.normalize('.nika/policies'))
        ) {
    ...
    const mockReadFile = vi.fn(
      async (
        ...
        if (
          ...
          nodePath
            .normalize(path)
            .includes(nodePath.normalize('.nika/policies/write.toml'))
        ) {
          return `
[[rule]]
    ...
    const mockReadFile = vi.fn(
      async (
        ...
        if (
          ...
          nodePath
            .normalize(path)
            .includes(nodePath.normalize('.nika/policies/safety.toml'))
        ) {
          return `
[[rule]]
toolName = "write_file"
    ...
    const mockReadFile = vi.fn(
      async (
        ...
        if (
          ...
          nodePath
            .normalize(path)
            .includes(nodePath.normalize('.nika/policies/invalid_safety.toml'))
        ) {
          return `
[[rule]]
toolName = "write_file"
