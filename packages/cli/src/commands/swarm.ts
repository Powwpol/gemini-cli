    ...
    yargs
      ...
      .command(
        ...
        async (argv) => {
          ...
          debugLogger.log(`Spawning Nika Worker: ${jobName}...`);
              ...
              template: {
                spec: {
                  containers: [
                    {
                      ...
                      volumeMounts: [
                        { name: 'vault-storage', mountPath: '/workspace' },
                      ],
                    },
                  ],
                  restartPolicy: 'Never',
                  volumes: [
                    {
                      name: 'vault-storage',
                      persistentVolumeClaim: { claimName: 'nika-vault-pvc' },
                    },
                  ],
                },
              },
          ...
          try {
            ...
            debugLogger.log(
              `Successfully spawned agent for task: "${argv.prompt}"`,
            );
          } catch (_e) {
            debugLogger.error(
              'Error connecting to Kubernetes cluster. Ensure K8s is enabled in Docker Desktop.',
            );
          }
        },
      )
      .command('list', 'List active agents in the swarm', {}, async () => {
        ...
        try {
          ...
          debugLogger.error('Failed to list swarm pods.');
        }
      })
      .demandCommand(1)
      .version(false),
