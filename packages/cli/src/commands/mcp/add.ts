...
import { debugLogger, type MCPServerConfig } from '@pulsai/nika-cli-core';
  ...
  if (transport === 'stdio') {
    debugLogger.warn(
      'Security Warning: Running MCP servers with stdio transport can expose inherited environment variables. ' +
        'While the Nika CLI redacts common API keys and secrets by default, you should only run servers from trusted sources.',
    );
  }
