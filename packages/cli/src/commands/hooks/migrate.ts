...
import { debugLogger, getErrorMessage } from '@pulsai/nika-cli-core';
...
export const migrateCommand: CommandModule = {
  command: 'migrate',
  describe: 'Migrate hooks from Claude Code to Nika CLI',
  builder: (yargs) =>
  ...
  handler: async (argv) => {
    ...
    if (args.fromClaude) {
      ...
      debugLogger.log(
        'Usage: nika hooks migrate --from-claude\n\nMigrate hooks from Claude Code to Nika CLI format.',
      );
    }
    await exitCli();
  },
