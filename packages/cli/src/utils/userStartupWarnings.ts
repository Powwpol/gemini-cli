...
import { homedir } from '@pulsai/nika-cli-core';
  ...
  check: async (workspaceRoot: string, settings: Settings) => {
    ...
    try {
      ...
      if (workspaceRealPath === homeRealPath) {
        ...
        return 'Warning you are running Nika CLI in your home directory.\nThis warning can be disabled in /settings';
      }
      return null;
    } catch (_err: unknown) {
...
const rootDirectoryCheck: WarningCheck = {
  ...
  check: async (workspaceRoot: string, _settings: Settings) => {
    try {
      ...
      const errorMessage =
        'Warning: You are running Nika CLI in the root directory. Your entire folder structure will be used for context. It is strongly recommended to run in a project-specific directory.';
