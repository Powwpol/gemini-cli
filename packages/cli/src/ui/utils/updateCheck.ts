...
import { getPackageJson, debugLogger } from '@pulsai/nika-cli-core';
  ...
  try {
    ...
    if (isNightly) {
      ...
      if (bestUpdate && semver.gt(bestUpdate, currentVersion)) {
        const message = `A new version of Nika CLI is available! ${currentVersion} → ${bestUpdate}`;
      ...
      if (latestUpdate && semver.gt(latestUpdate, currentVersion)) {
        const message = `Nika CLI update available! ${currentVersion} → ${latestUpdate}`;
