  ...
  describe('/memory list', () => {
    ...
    beforeEach(() => {
      ...
      vi.mocked(listMemoryFiles).mockImplementation((config) => {
        ...
        let content;
        if (fileCount > 0) {
          content = `There are ${fileCount} NIKA.md file(s) in use:\n\n${filePaths.join('\n')}`;
        } else {
          content = 'No NIKA.md files in use.';
        }
        return {
          type: 'message',
          messageType: 'info',
    ...
    it('should display a message if no NIKA.md files are found', async () => {
    ...
    it('should display the file count and paths if they exist', async () => {
      ...
      const filePaths = ['/path/one/NIKA.md', '/path/two/NIKA.md'];
      ...
      expect(mockContext.ui.addItem).toHaveBeenCalledWith(
        {
          type: MessageType.INFO,
          text: `There are 2 NIKA.md file(s) in use:\n\n${filePaths.join('\n')}`,
        },
        expect.any(Number),
      );
    });
