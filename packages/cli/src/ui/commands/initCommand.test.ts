...
import type { SubmitPromptActionReturn } from '@pulsai/nika-cli-core';
...
describe('initCommand', () => {
  ...
  const geminiMdPath = path.join(targetDir, 'NIKA.md');
  ...
  it('should inform the user if NIKA.md already exists', async () => {
  ...
  it('should create NIKA.md and submit a prompt if it does not exist', async () => {
    ...
    expect(mockContext.ui.addItem).toHaveBeenCalledWith(
      {
        type: 'info',
        text: 'Empty NIKA.md created. Now analyzing the project to populate it.',
      },
      expect.any(Number),
    );
