...
import { performInit } from '@pulsai/nika-cli-core';
...
import type { CommandActionReturn, Config } from '@pulsai/nika-cli-core';
  ...
  describe('execute', () => {
    it('handles info from performInit', async () => {
      vi.mocked(performInit).mockReturnValue({
        type: 'message',
        messageType: 'info',
        content: 'NIKA.md already exists.',
        ...
        expect.objectContaining({
          ...
          status: expect.objectContaining({
            ...
            message: expect.objectContaining({
              parts: [{ kind: 'text', text: 'NIKA.md already exists.' }],
            }),
          }),
        }),
      );
    ...
    describe('when handling submit_prompt', () => {
      beforeEach(() => {
        vi.mocked(performInit).mockReturnValue({
          type: 'submit_prompt',
          content: 'Create a new NIKA.md file.',
        } as CommandActionReturn);
      });
      ...
      it('writes the file and executes the agent', async () => {
        ...
        expect(fs.writeFileSync).toHaveBeenCalledWith(
          path.join(mockWorkspacePath, 'NIKA.md'),
          '',
          'utf8',
        );
          ...
          expect.objectContaining({
            userMessage: expect.objectContaining({
              parts: expect.arrayContaining([
                expect.objectContaining({
                  text: 'Create a new NIKA.md file.',
                }),
              ]),
              metadata: {
                coderAgent: {
